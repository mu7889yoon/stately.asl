...
import { createDefaultRegistry, PluginRegistry } from "./plugins/index.js";
import { parseFile, analyze as analyzeFile } from "./parser/index.js";
import { buildCFGFromSourceFile } from "./cfg/index.js";
...
function runTranspile(options: TranspileOptions): TranspileResult {
  ...
  const { cfg } = buildCFGFromSourceFile({
...
export async function analyze(options: AnalyzeOptions): Promise<AnalyzeResult> {
  ...
  const result = analyzeFile({ ...options, registry });
  return {
    ok: result.ok,
